...
const searchFlightsPostController = async (req, res) => {
    let { timing, boardingPoint, destinationPoint } = req.body
    // console.log(req.body)
    boardingPoint = boardingPoint.charAt(0).toUpperCase()+boardingPoint.slice(1).toLowerCase()
    destinationPoint = destinationPoint.charAt(0).toUpperCase()+destinationPoint.slice(1).toLowerCase()
    const flightData = await flightModel.find({ timing: timing, reserved: { $lt: 60 }, from: boardingPoint, to: destinationPoint }).exec()
    // console.log(flightData, boardingPoint, destinationPoint)
